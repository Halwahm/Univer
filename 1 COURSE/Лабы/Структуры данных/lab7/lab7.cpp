...
void Menu()
{
	...
	cout << "!!! 9 - сортировка клиентов по имени" << endl;
	cout << "!!! 10 - сортировка торговых точек по этажу" << endl;
...
void SortbyNames(client client[]) 
{
	cout << "\t\t\t\t\tОтсортированные по алфавиту данные\n";
		...
		for (int i = 0; i < N; i++) {
			...
			cout << client[i].name << ' ' << client[i].address << ' ' << client[i].person
				<< ' ' << client[i].check << ' ' << client[i].phone << endl;
		}
}

void sortbyFloor(rent point[]) {
	...
	for (int i = 0; i < N; i++) {
		smallest_id = i;
		for (int j = i + 1; j < N; j++) {
			if ((*(point[smallest_id].floor)) > (*(point[j].floor))) {
				smallest_id = j;
			}
			swap(point[smallest_id].floor, point[i].floor);
			swap(point[smallest_id].conditioner, point[i].conditioner);
			swap(point[smallest_id].area, point[i].area);
			swap(point[smallest_id].price, point[i].price);
		}
		cout << point[i].floor << ' ' << point[i].area << ' ' << point[i].price
			<< ' ' << point[i].conditioner << endl;
	}
	cout << "Ниже этажом - удобнее!" << endl;
}
...
void dop1(rent point[])
{
	...
	for (int i = 1; i < N; i++)
	{
		key = point[i].price;
		j = i - 1;
		while (j >= 0 && point[j].price > key)
		{
			point[j + 1].price = point[j].price;
			j--;
		}
		point[j + 1].price = key;

		cout << "С меньшей стоимостью аренды: " << point[i].floor << ' ' << point[i].area << ' ' << point[i].price
			 << ' ' << point[i].conditioner << endl;
	}
}

void dop2(rent point[]) { //вставка
	char* key;
	int i = 0;
	for (int j = 1; j < N; j++) {
		key = point[j].price;
		i = j - 1;
		while (i >= 0 && point[i].price > key) {
			point[i + 1] = point[i];
			i = i - 1;
		point[i + 1].price = key;
		}
	...
	cout << " Самая высокая арендная плата";

	for (int k = 0; k < N; k++)
	{
		cout << ' ' << point[k].price << ' ' << endl;
	}
}

void dop3(rent point[]) //выбором
{
	int smallest_id;
...
int main()
{
	...
	do {
		...
		case 9: SortbyNames(client); Menu(); break;
		case 10: sortbyFloor(point); Menu(); break;
		case 11: cout << endl << "До свидания!" << endl; break;
		case 12: dop1(point); Menu(); break;
		case 13: dop2(point); Menu(); break;
		case 14: dop3(point); Menu(); break;
		}
	} while (choose != 11);
}
