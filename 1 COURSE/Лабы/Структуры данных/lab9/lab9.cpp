...
void Menu()
{
	...
	cout << "9 - поиск номера обращения клиента (id)" << endl;
	cout << "10 - интерпретирующий поиск клиента" << endl;
	cout << "11 - бинарный поиск клиента" << endl;
...
void client_entry(client client[])
{
	...
	for (short i = 0; i < num; i++)
	{
		...
		cout << "Введите день обращения:\n";
		file2 << "\nДень обращения клиента: ";
		cin >> client[i].date;
		file2 << client[i].date;
		CountA[i] = client[i].date;
		cout << endl;
		client[i].IDc = 1 + ((i + 1) * 2);
	}
}
...
void SearchO(client* client)
{
	int dot;
	cout << "Введите номер клиента (id обращения): \n";
	cin >> dot;
	for (int i = 0; i < N; i++)
	{
		if ((dot * 2) + 1 == client[i].IDc)
		{
			cout << "Имя : " << client[i].name << "\n";
		}
	}
}
...
int main()
{
	...
	do {
		...
		case 9: SearchO(client); Menu(); break;
		case 10: search_int(); Menu(); break;
		case 11: binary(); Menu(); break;
		case 99: cout << endl << "До свидания!" << endl; break;
		}
	} while (choose != 99);
}
*/