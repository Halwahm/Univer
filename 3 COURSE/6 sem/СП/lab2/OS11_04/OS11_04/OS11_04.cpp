#pragma comment(lib, "D:\\6sem\\SP\\Labs\\LAB03\\OS11_04\\OS11_04\\OS11HTAPI.lib")
...
int main()
{
	...
	try
	{
		...
		while (true)
		{
			...
			{
				char infobuff[555] = "\nпопытка обновления в хранилище элемента с ключом ";
				strcat(infobuff, Keys[i-1]);
				cout << infobuff;
				LoggInfo(infobuff);

				Element* found_element = NULL;
				found_element = Get(ht, new Element(Keys[i - 1], strlen(Keys[i - 1])));

				if (found_element != NULL)
				{
					strcpy(infobuff, " - успех");
	
					unsigned long payloadvalue = *((unsigned long*)(found_element->payload));

					strcat(infobuff, " - текущее значение payload: ");
					_ultoa(payloadvalue, infobuff + strlen(infobuff), 10);

					payloadvalue++;

					strcat(infobuff, " - новое значение payload: ");
					_ultoa(payloadvalue, infobuff + strlen(infobuff), 10);

					if (Update(ht, found_element, &payloadvalue, sizeof(payloadvalue)))
					{
						strcat(infobuff, " - успешно обновлено");
					}
					else
					{
						strcat(infobuff, " - отказ обновления: ");
...
bool LoggInfo(char * infostr)
{
	...
	try
	{
		logfile = CreateFile("D:\\6sem\\SP\\Labs\\LAB03\\temp\\OS11_04.log",
