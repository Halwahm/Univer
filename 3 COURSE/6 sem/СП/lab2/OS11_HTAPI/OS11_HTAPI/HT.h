#pragma once
#include <Windows.h>
//обьявление функция API для доступа к хранилищу данных в оперативной памяти в формате ключ/значение
...
struct HTHANDLE    // блок управления HT
{
	...
	LPVOID  Addr = NULL;                   // Addr != NULL, если mapview выполнен  
...
extern "C" __declspec(dllexport) HTHANDLE*   Open     //  открыть HT - через OpenFileMapping()
(
...
__declspec(dllexport) char* GetLastError  // получить сообщение о последней ошибке
(
