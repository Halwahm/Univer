...
char* getPseudoRandomKey7(char*);
char* getRandomKey_Test(char*, int);

bool LoggInfo(char *);

// поддержка вызова функции из DLL

typedef bool(*HT_Insert)(HTHANDLE *, Element *);   // объявляем указатель на функцию из dll
typedef HTHANDLE*(*HT_Open)(const char*, char*);  
...
int main()
{
	...
	HINSTANCE hDLL = LoadLibrary("OS11HTAPI.dll"); // загружаем dll в память процесса

	if (hDLL == NULL)
	{
		cout << "ошибка загрузки OS11HTAPI.dll";
		return 1;
	}

	HT_Insert ht_insert_f = (HT_Insert)GetProcAddress(hDLL, "Insert");
	HT_Open ht_open_f = (HT_Open)GetProcAddress(hDLL, "Open");
	...
	try
	{
		...
		ht = ht_open_f(HTFileName, errorInfo);
		...
		for (int i = 0; i < keys_QTT; i++)
		{
			...
			// getRandomKey(Keys[i], ht->htc.MaxKeyLength); !!!!!!!!!!!!!!!! переключаем тут 
		...
		while (true)
		{
			for (int i = 1; i <= keys_QTT; i++)
			{
				...
				if (ht_insert_f(ht, new Element(Keys[i-1], strlen(Keys[i-1]), &payload, sizeof(payload))))
				{
					char infobuff[255] = "\nв хранилище успешно помещен элемент с ключом ";
					strcat(infobuff, Keys[i-1]);
					strcat(infobuff, " и данными ");
					_itoa(payload, infobuff + strlen(infobuff), 10);
				...
				{
					char infobuff[255] = "\n отказ помещения в хранилище элемента с ключом ";
					strcat(infobuff, Keys[i-1]);
					strcat(infobuff, " и данными ");
					_itoa(payload, infobuff + strlen(infobuff), 10);
					strcat(infobuff, " - ошибка: ");
...
bool LoggInfo(char * infostr)
{
	...
	try
	{ 
		logfile = CreateFile("D:\\6sem\\SP\\Labs\\LAB03\\temp\\OS11_02.log",
