...
//---------------------------------------------------------------------------------
double &CMatrix::operator()(int i) //для вектора
// i - номер строки
{
if (n_cols>1)     
   {
...
//---------------------------------------------------------------------------------
CMatrix CMatrix::operator=(const CMatrix& M) //копирование
...
//---------------------------------------------------------------------------------
CMatrix::CMatrix(const CMatrix &M) // глубокое копирование - новый объект
{
	n_rows=M.n_rows;
	n_cols=M.n_cols;
	array=new double*[n_rows];
...
// возвращает строку заданным параметрам столбцов
{
	int b1=(k>=0)&&(k<n_rows);
	int b2=(n>=0)&&(n<=m);
	int b3=(m>=0)&&(m<n_cols);
...
// возвращает столбец по заданным параметрам
