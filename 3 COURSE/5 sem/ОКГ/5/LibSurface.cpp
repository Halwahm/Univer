...
//Определяет номер области  для  рисования.
{
	...
	if ((yView <= y1) && (yView <= y2))return 1; //верх право
	if ((yView > y2) && (yView < y1))return 2; //низ лево
	if ((yView >= y1) && (yView >= y2))return 3; //верх лево
	if ((yView > y1) && (yView < y2))return 4; //низ право
}
