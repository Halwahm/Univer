...
bool GetServer1(char* name, short port, sockaddr* from, int* flen);
bool GetServer2(char* name, short port, sockaddr* from, int* flen);
...
void main()
{
    ...
    try
    {
        ...
        if (GetServer1(name, sock, (sockaddr*)&from, &lfrom))
        {
    ...
    try
    {
        ...
        if ((sock = socket(AF_INET, SOCK_DGRAM, NULL)) == INVALID_SOCKET)
            throw SetErrorMsgText("socket: ", WSAGetLastError());
        if (GetServer2(name, sock, (sockaddr*)&from, &lfrom))
        {
            cout << "Connected server: " << endl;
            cout << "IP: " << inet_ntoa(from.sin_addr) << endl;
            cout << "Port: " << htons(from.sin_port) << endl;
...
bool GetServer2(char* name, short port, sockaddr* from, int* flen)
{
    ...
    ((SOCKADDR_IN*)from)->sin_family = AF_INET;
    ((SOCKADDR_IN*)from)->sin_port = htons(2001);
    ((SOCKADDR_IN*)from)->sin_addr.s_addr = INADDR_BROADCAST;
    int lbuf;
    char ibuf[50];
    int optval = 1;
